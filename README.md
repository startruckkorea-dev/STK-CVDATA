...
폭 880px 이하에서는 좌측 메뉴가 화면을 차지하는 대신 **왼쪽 위 버튼으로 여는 서랍(drawer)** 으로 바뀌고,
제목·필터·KPI 를 담은 헤더의 고정(sticky)이 풀립니다 — 고정해 두면 차트에 남는 세로 공간이 거의 없습니다.
차트는 720px 이하에서 높이·여백·라벨 크기를 줄이고, 12개월처럼 촘촘한 축은 라벨을 한 칸 걸러 표시합니다.
넓은 표는 줄여 찍는 대신 카드 안에서 좌우로 스크롤합니다. 터치 화면에서는 차트를 정적으로 그립니다 — 손가락 드래그를 Plotly 가 확대/이동으로 읽어 스크롤하려다 차트 축이 바뀌는 일을 막기 위함이며, 값은 이미 마크 위에 찍혀 있어 잃는 정보가 없습니다. iOS·안드로이드 구분 없이 창 폭으로만 판단하므로
태블릿 가로/세로 전환도 그대로 따라갑니다.
